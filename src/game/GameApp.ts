...
import { createWaterfallTexture, createWaterCurrentTexture } from './environment/water/WaterfallTexture';
...
// ... imports
  ...
  constructor(container: HTMLElement) {
    ...
    this.boundTogglePause = (e: any) => {
        console.log(`App: [Instance ${this.instanceId}] Received TogglePause event`, e?.detail);
        this.togglePause(e?.detail);
    };
    this.boundSave = (e: any) => {
        const inv = e?.detail?.inventory || e?.inventory; // Robust extraction
        console.log(`App: [Instance ${this.instanceId}] Received Save event. Inv:`, inv);
        this.saveGame(inv);
    };
    this.boundLoad = () => {
  ...
  private boundTogglePause: any;
  private boundSave: any;
  private boundLoad: any;
  private boundNew: any;

  public newGame(): void {
      console.log(`App: [Instance ${this.instanceId}] Starting New Game procedure...`);
      ...
      try {
          ...
          // 2. Clear Visuals
          console.log(`App: [Instance ${this.instanceId}] Clearing building visuals...`);
          this.visuals.forEach((v, k) => {
          ...
          console.log(`App: [Instance ${this.instanceId}] Re-initializing terrain...`);
          this.initTerrain();
          ...
          console.log(`App: [Instance ${this.instanceId}] New Game procedure completed!`);
      } catch (err) {
  ...
  public destroy() {
    ...
    console.log(`App: [Instance ${this.instanceId}] Listeners removed and destroyed.`);
  }
  ...
  private initTerrain() {
    ...
    const { material: grassMaterial, uniforms: grassUniforms } = createGrassMaterial();
    this.grassUniforms = grassUniforms;
    
    this.instancedGrass = new THREE.InstancedMesh(grassGeo, grassMaterial, totalBlades);
    ...
    for (let y = 0; y < WORLD_HEIGHT; y++) {
      for (let x = 0; x < WORLD_WIDTH; x++) {
        ...
        const topMat = tile.isSand() ? beachMat : (tile.isWater() ? waterMat : grassMat);
        const sideMat = tile.isSand() ? sandMat : (tile.isWater() ? waterMat : grassMat);
        
        // Face check helper
        const getFaceMat = (nx: number, ny: number, defaultMat: THREE.Material) => {
        ...
        if (tile.isWater()) {
          ...
          if (isAtEdge) {
            ...
            if (isLeft) waterTopMat = new THREE.MeshLambertMaterial({ map: this.currentTextures.west, transparent: true, opacity: 0.8 });
            if (isRight) waterTopMat = new THREE.MeshLambertMaterial({ map: this.currentTextures.east, transparent: true, opacity: 0.8 });
            if (isTop) waterTopMat = new THREE.MeshLambertMaterial({ map: this.currentTextures.north, transparent: true, opacity: 0.8 });
            if (isBottom) waterTopMat = new THREE.MeshLambertMaterial({ map: this.currentTextures.south, transparent: true, opacity: 0.8 });
              ...
              for (let i = 0; i < this.BLADES_PER_TILE; i++) {
                   if (idx < this.initialGrassMatrices.length) {
                       if (shouldHide) {
                           ...
                           this.instancedGrass.setMatrixAt(idx, this.initialGrassMatrices[idx]);
                       }
                   }
                   idx++;
              }
  ...
  public loadGame(): void {
      ...
      try {
          ...
          if (Array.isArray(inventoryData)) {
              ...
              finalInventory = Array(10).fill(null).map(() => ({ type: null, count: 0 }));
              let idx = 0;
          ...
          window.dispatchEvent(new CustomEvent('GAME_LOAD_INVENTORY', { detail: finalInventory }));
      } catch (err: any) {
