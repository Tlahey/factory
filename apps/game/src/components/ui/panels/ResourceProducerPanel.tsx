 ...
 * Used by Extractor, Sawmill, and similar buildings
 */
interface ResourceProducerPanelProps {
  /** The building entity */
  ...
  /** What resource this building produces (for display) */
  resourceType?: string;
  /** Rate unit label */
  rateLabel?: string;
  /** Drag handlers */
...
/**
 * Get status info helper for resource producers
 */
export function getResourceProducerStatusInfo(
  hasPowerSource: boolean,
  operationStatus: string,
  powerStatus: string,
  isActive: boolean,
...
/**
 * Shared panel for buildings that extract/produce resources
 * (Extractor, Sawmill, etc.)
 *
 * Displays:
 * - Status indicator
 * - Extraction rate
 ...
 * - Output buffer
 */
export function ResourceProducerPanel({
  building,
  resourceType,
  rateLabel,
  onDragStart,
  onDragEnd,
  onDragOver,
}: ResourceProducerPanelProps) {
  const { t } = useTranslation();

  const { status, color } = getResourceProducerStatusInfo(
  ...
  // Get extraction rate (items per minute)
  const extractionRate = building.getExtractionRate
    ? building.getExtractionRate() * 60
    : 0;

  // Determine output type from buffer or prop
  const outputType =
    resourceType ||
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="p-3 bg-black/20 rounded-lg border border-white/5">
            ...
            <div className="text-lg font-mono font-bold text-white capitalize">
              {t(`resource.${outputType}`) || outputType}
            </div>
          </div>
