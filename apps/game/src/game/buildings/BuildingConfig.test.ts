...
describe("Building Configuration Integrity", () => {
  it("should have an id for every building that matches its key", () => {
    Object.entries(BUILDINGS).forEach(([_key, config]) => {
      ...
      describe(`Locale: ${name}`, () => {
        ...
        it("should have translation keys for every building", () => {
          Object.values(BUILDINGS).forEach((config) => {
            ...
            if (!buildingLocales) {
              throw new Error(
                `Missing translations for building '${buildingKey}' in locale '${name}'`,
              );
            }

            expect(
              buildingLocales.name,
              `Missing name for ${buildingKey} in ${name}`,
            ).toBeDefined();
            expect(
              buildingLocales.name.length,
              `Empty name for ${buildingKey} in ${name}`,
            ).toBeGreaterThan(0);

            expect(
              buildingLocales.description,
              `Missing description for ${buildingKey} in ${name}`,
            ).toBeDefined();
            expect(
              buildingLocales.description.length,
              `Empty description for ${buildingKey} in ${name}`,
            ).toBeGreaterThan(0);
          });
        });
      });
