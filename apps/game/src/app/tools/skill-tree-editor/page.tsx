...
import { SKILL_TREE, SkillNode, SkillNodeType } from "@/game/buildings/hub/skill-tree/SkillTreeConfig";
import { ChevronLeft, Save, Copy, Plus } from "lucide-react";
...
export default function SkillTreeEditor() {
    ...
    const handleDragStart = (e: React.MouseEvent, nodeId: string) => {
        ...
        // Calculate offset relative to grid cell
        // Mouse relative to node
        // Current node screen pos
        // Just snap to grid on drop, for now simple dragging
    };

    const handleMouseMove = (e: React.MouseEvent) => {
        if (!draggingNode) return;

        // This is a naive drag implementation that doesn't account for pan/zoom or rect bounds well
        // A better way for a grid editor is usually to detect grid cell hover
        // But for now, let's just use simple discrete updates on click/drop or just simulate visual drag

        // Actually, let's implement click-to-move style or simple grid snap
    };
    ...
    const handleExport = () => {
        ...
        navigator.clipboard.writeText(`export const SKILL_TREE: SkillNode[] = ${json};`);
        ...
        <div className="min-h-screen bg-gray-950 text-white flex flex-col">
            ...
            <div className="h-16 border-b border-gray-800 bg-gray-900 px-6 flex items-center justify-between z-10 relative shadow-md">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-bold text-lg">Skill Tree Editor</h1>
                </div>
            ...
            <div className="flex-1 overflow-auto bg-[radial-gradient(#333_1px,transparent_1px)] [background-size:20px_20px] relative p-8">
                ...
                >
                    ...
                    {nodes.map(node => (
                        <div
                            key={node.id}
                            className={`absolute w-32 h-32 flex flex-col items-center justify-center p-2 rounded-xl border-2 cursor-move transition-shadow z-10
                    ...
                    `}
                        ...
                        >
                            <div className="flex gap-1 mb-2 absolute -top-8 bg-gray-900 p-1 rounded-lg border border-gray-700 opacity-0 group-hover:opacity-100 transition-opacity">
                                {/* Controls */}
                            ...
                            <span className="text-xs font-bold text-center px-1 break-words w-full">{node.id}</span>
