...
export class InputSystem {
  ...
  private onDeleteHover?: (
    target: {
      type: "cable" | "building";
      id: string;
      x?: number;
      y?: number;
      cable?: { x1: number; y1: number; x2: number; y2: number };
    } | null,
  ) => void;
  ...
  constructor(
    ...
    useGameStore.subscribe((state) => {
      ...
      if (
        state.cameraAzimuth !== this.targetAzimuth ||
        state.cameraElevation !== this.targetElevation
      ) {
  ...
  private handleKeyDown(e: KeyboardEvent) {
    ...
    if (e.key === "s") {
      ...
      useGameStore
        .getState()
        .setSelectedBuilding(current === "select" ? null : "select");
    }
    if (e.key === "Delete" || e.key === "Backspace") {
      const current = useGameStore.getState().selectedBuilding;
      useGameStore
        .getState()
        .setSelectedBuilding(current === "delete" ? null : "delete");
    }
  }
  ...
  private onPointerMove(event: PointerEvent) {
    ...
    if (this.isDraggingCable && this.cableStart && intersection) {
      ...
      const startB = this.world.getBuilding(
        this.cableStart.x,
        this.cableStart.y,
      );
      ...
      if (startB?.getType() === "hub") {
        const currentConns = this.world.getBuildingConnectionsCount(startB);
      ...
      if (endB) {
        if (endB.getType() === "hub") {
          const endConns = this.world.getBuildingConnectionsCount(endB);
          if (endConns >= 1) isValid = false;
        } else if (endB.getType() === "electric_pole") {
          const endConns = this.world.getConnectionsCount(
            intersection.x,
            intersection.y,
          );
  ...
  private handleCableEnd(endX: number, endY: number) {
    ...
    // Hub Limit
    if (startB?.getType() === "hub") {
      if (this.world.getBuildingConnectionsCount(startB) >= 1) isValid = false;
    }
    if (endB?.getType() === "hub") {
      if (this.world.getBuildingConnectionsCount(endB) >= 1) isValid = false;
    }
    ...
    if (isValid) {
      ...
      if (added) {
        if (cableConfig && cableConfig.cost) {
          removeResources(cableConfig.cost);
        }
  ...
  private handleConveyorDragEnd(endX: number, endY: number) {
    ...
    // Actually we already validated above.
    const count = path.length;
    const singleCost = conveyorConfig?.cost || {};
    const totalCost: Record<string, number> = {};
    for (const [res, amt] of Object.entries(singleCost)) {
    ...
    if (!hasResources(totalCost)) {
      console.log("Insufficient resources for conveyors");
      this.cancelConveyorDrag();
      return;
    }
