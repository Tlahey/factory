 ...
 */
export function calculateTurnType(flowIn: Direction, flowOut: Direction): ConveyorTurnType {
 ...
 */
export function getDirectionBetweenPoints(fromX: number, fromY: number, toX: number, toY: number): Direction | null {
    const dx = toX - fromX;
    const dy = toY - fromY;
...
export function findOutputDestination(
    ...
    for (const checkDir of directions) {
        ...
        if (excludeDirection && checkDir === getOppositeDirection(excludeDirection)) {
            continue;
        }
...
export function determineFlowInputDirection(
    ...
    for (const checkDir of directions) {
        ...
        const offset = getDirectionOffset(checkDir);
        const neighbor = world.getBuilding(conveyorX + offset.dx, conveyorY + offset.dy);
        
        if (!neighbor) continue;
        
        const neighborType = neighbor.getType();
        const neighborDirection = neighbor.direction as 'north' | 'south' | 'east' | 'west';
