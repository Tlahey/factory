...
vi.mock('three', async () => {
    ...
    return {
        ...
        Group: class {
            ...
            getObjectByName(name: string) { return this.children.find(c => c.name === name); }
        },
        Mesh: class {
            rotation = { x: 0, y: 0, z: 0 };
            position = { x: 0, y: 0, z: 0, set: function(x: number, y: number, z: number) { this.x=x; this.y=y; this.z=z; } };
...
describe('IOArrowHelper', () => {
    ...
    test('Arrows are created relative to North regardless of building rotation', () => {
        ...
        const groupNorth = createIOArrows(northExtractor as BuildingEntity & IIOBuilding);
        const arrowNorth = groupNorth.children[0];
        
        const eastExtractor = new Extractor(0, 0, 'east');
        const groupEast = createIOArrows(eastExtractor as BuildingEntity & IIOBuilding);
