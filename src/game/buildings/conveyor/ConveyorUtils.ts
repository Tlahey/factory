...
export class ConveyorUtils {
     ...
     */
    public static getItemPosition(x: number, y: number, direction: ConveyorDir, type: ConveyorType, progress: number): THREE.Vector3 {
        ...
        if (type === 'straight') {
            ...
            switch (direction) {
                case 'north': lz = 0.5 - progress; break; // Enter at Z=0.5, Exit Z=-0.5
                case 'south': lz = -0.5 + progress; break; // Enter Z=-0.5, Exit Z=0.5
                case 'east':  lx = -0.5 + progress; break; // Enter X=-0.5
                case 'west':  lx = 0.5 - progress; break;  // Enter X=0.5
            }
            ...
            if (direction === 'north') { // Output North (-Z)
                if (isLeft) { 
                    ...
                    pdx = -0.5; pdz = -0.5;
                    return ConveyorUtils.getArcPos(cx, cz, h, pdx, pdz, Math.PI / 2, 0, progress);
                } else {
                    ...
                    pdx = 0.5; pdz = -0.5;
                    return ConveyorUtils.getArcPos(cx, cz, h, pdx, pdz, Math.PI / 2, Math.PI, progress);
                }
            }
            else if (direction === 'south') { // Output South (+Z)
               if (isLeft) {
            ...
            else if (direction === 'east') { // Output East (+X)
                if (isLeft) {
                    ...
                    pdx = 0.5; pdz = -0.5;
                    return ConveyorUtils.getArcPos(cx, cz, h, pdx, pdz, Math.PI, Math.PI/2, progress);
                } else {
                    ...
                    pdx = 0.5; pdz = 0.5;
                    return ConveyorUtils.getArcPos(cx, cz, h, pdx, pdz, Math.PI, 1.5 * Math.PI, progress);
                }
            }
            else if (direction === 'west') { // Output West (-X)
                if (isLeft) {
                    ...
                    pdx = -0.5; pdz = 0.5;
                    return ConveyorUtils.getArcPos(cx, cz, h, pdx, pdz, 0, -Math.PI/2, progress);
                } else {
    ...
    private static getArcPos(cx: number, cz: number, y: number, pdx: number, pdz: number, startAng: number, endAng: number, t: number): THREE.Vector3 {
    ...
    public static getItemRotation(direction: ConveyorDir, type: ConveyorType, progress: number): number {
        if (type === 'straight') {
             switch (direction) {
                case 'north': return Math.PI; // Face North (-Z) -> Wait, atan2(0, -1) = PI. Logic check: dx=0, dz=-1. atan2(0,-1)=PI.
            ...
            if (direction === 'north') { // Output North
                if (isLeft) { // West -> North (Left)
                    startAng = Math.PI / 2; endAng = 0;
                } else { // East -> North (Right)
                    startAng = Math.PI / 2; endAng = Math.PI;
                }
            }
            else if (direction === 'south') { // Output South
               if (isLeft) { // East -> South (Left)
                   startAng = -Math.PI/2; endAng = -Math.PI;
               } else { // West -> South (Right)
                   startAng = -Math.PI/2; endAng = 0;
               }
            }
            else if (direction === 'east') { // Output East
                if (isLeft) { // North -> East (Left)
                     startAng = Math.PI; endAng = Math.PI/2;
                } else { // South -> East (Right)
                    startAng = Math.PI; endAng = 1.5 * Math.PI;
                }
            }
            else if (direction === 'west') { // Output West
                if (isLeft) { // South -> West (Left)
                    startAng = 0; endAng = -Math.PI/2;
                } else { // North -> West (Right)
                    startAng = 0; endAng = Math.PI/2;
                }
            }
