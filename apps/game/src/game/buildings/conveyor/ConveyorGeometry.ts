...
export function createConveyorModel(
  ...
  if (type === "straight") {
    ...
    const createRail = (radius: number) => {
      const railArc = new THREE.EllipseCurve(
        -0.5,
        0.5,
        radius,
        radius,
      ...
      return new THREE.Mesh(
        new THREE.ExtrudeGeometry(rShape, {
          steps: 12,
          bevelEnabled: false,
          extrudePath: rPath,
        }),
    ...
    // 3. Bed (Rotated)
    const bedArc = new THREE.EllipseCurve(
      -0.5,
      0.5,
      0.5,
      0.5,
    ...
    const bPath = new THREE.CatmullRomCurve3(
      bedArc.getPoints(12).map((p) => new THREE.Vector3(p.x, 0, p.y)),
    );
    ...
    const bedMesh = new THREE.Mesh(
      new THREE.ExtrudeGeometry(bedShape, {
        steps: 12,
        bevelEnabled: false,
        extrudePath: bPath,
      }),
