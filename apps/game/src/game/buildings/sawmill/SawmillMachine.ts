...
export const sawmillMachine = setup({
  types: {
    context: {} as {
      building: Sawmill;
      stabilityTimer: number;
    },
    input: {} as { building: Sawmill },
    events: {} as { type: "TICK"; delta: number; world: IWorld },
  },
  actions: {
    tickSawmill: ({ context, event }) => {
      ...
      if (canMine && building.accumTime >= interval) {
        if (resourceTile) {
          ...
          const resourceType = resourceTile.getResourceType(); // "wood"
