...
export default function HUD() {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'i') { // Changed from Tab to i
    ...
    const handleDrop = (e: React.DragEvent, targetIndex: number) => {
        ...
        if (source === 'inventory') {
            ...
            window.dispatchEvent(new CustomEvent('GAME_INVENTORY_DROP', {
                detail: {
                    source: 'chest',
            ...
            >
                ...
                <div className="mb-4 pb-2 border-b border-white/10 flex items-center justify-between">
                    <span className="text-white font-bold text-sm tracking-widest uppercase flex items-center gap-2">
                        ...
                        <span className="bg-gradient-to-r from-amber-400 to-amber-200 bg-clip-text text-transparent">Inventory</span>
                    </span>
                        ...
                        >
                            ...
                            {slot.type && (
                                <div className="w-14 h-14 pointer-events-none">
                                    <ModelPreview type="item" id={slot.type} width={56} height={56} static seed={index * 100} />
                                </div>
                            )}
                            ...
                            {slot.type && (
                                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-slate-900 text-white text-sm rounded-lg shadow-xl border border-white/20 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-2 group-hover:translate-y-0 whitespace-nowrap z-50 pointer-events-none">
                                    <div className="font-bold capitalize mb-0.5 text-amber-300">{slot.type}</div>
                                    <div className="text-[10px] text-gray-400 uppercase tracking-wide">Quantity: <span className="text-white font-mono">{slot.count}</span></div>
                                    {/* Arrow */}
