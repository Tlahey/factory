      ...
      <div className="flex-1 overflow-auto bg-[radial-gradient(#333_1px,transparent_1px)] [background-size:20px_20px] relative p-8">
        ...
        >
          ...
          {Array.from({ length: maxX }).map((_, i) => (
            <div
              key={`v-${i}`}
              className="absolute top-0 bottom-0 w-px bg-gray-800/30 pointer-events-none"
              style={{ left: i * GRID_CELL_SIZE + GRID_PADDING }}
            />
          ))}
          {Array.from({ length: maxY }).map((_, i) => (
            <div
              key={`h-${i}`}
              className="absolute left-0 right-0 h-px bg-gray-800/30 pointer-events-none"
              style={{ top: i * GRID_CELL_SIZE + GRID_PADDING }}
            />
          ))}
          ...
          {nodes.map((node) => (
            ...
            >
              ...
              <span className="text-xs font-bold text-center px-1 break-words w-full overflow-hidden text-ellipsis">
                {node.id}
              </span>
              ...
              {node.level > 0 && (
                <span className="text-[10px] bg-indigo-500/20 text-indigo-300 px-1 rounded mt-1">
                  Lv.{node.level}
                </span>
              )}
            </div>
